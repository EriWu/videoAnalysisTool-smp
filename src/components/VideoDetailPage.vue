<template>
  <div>
    ...
    <div v-else>
      <!-- 遍历 textAnnotations -->
      <div v-for="(segments, key) in textAnnotations" :key="key" class="annotation-item">
...
export default {
  ...
  async created() {
    ...
    try {
      const response = await fetch(`https://us-central1-seemeplease-3e926.cloudfunctions.net/getVideoSignedUrl?fileName=${this.videoName}`);
